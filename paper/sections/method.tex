...
which is then scored by two objectives: fabric efficiency
and texture alignment across seams.
...
fixed seam-straightness regularization.
...
\[
\mathbf{F}(\mathbf{x}) = [w_1 f_1,\; w_2 f_2]^T,
\]
where $w_1, w_2 \geq 0$ are designer-specified weights and both
...
discrete $\boldsymbol{\rho}, \boldsymbol{\kappa}, h$,
...
\begin{itemize}
    ...
    \item $h$: coin flip between the two parent values.
\end{itemize}
