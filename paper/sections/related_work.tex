...
wasted material is known as the irregular strip packing or nesting problem.
...
Burke et al.~\cite{BurkeHKW06} introduce the bottom-left-fill heuristic,
which places each piece at the lowest, leftmost feasible position on the
strip; this heuristic forms the basis of our nesting engine.
...
specifically with simulated annealing, demonstrating the practical relevance
of metaheuristics for garment nesting under industrial constraints.
Gomes and Oliveira~\cite{GomesO06} combine simulated annealing with linear
programming to improve local compaction, achieving significant gains on
standard benchmarks.
...
alone; our nesting component deliberately uses a simpler heuristic because
the complexity of our problem lies in the joint objectives, and any
NFP-based engine could in principle be substituted without affecting the
other contributions.
...
Our method addresses both limitations: fabric roll constraints and material
efficiency are explicit objectives, and fit deviation is quantified and
minimised as a separate objective rather than treated as a side effect.
...
We adopt a simplified variant of this selection scheme, using fitness sum
as the tiebreaker in place of crowding distance, which is sufficient for
...
a unified algorithm.
Our design follows this principle: Gaussian perturbation for the continuous
seam parameters, uniform resampling for the discrete phase and orientation
variables, and swap mutation for the permutation-valued nesting order.
