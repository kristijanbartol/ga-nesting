...
B4 is a standard off-the-shelf continuous optimiser, CMA-ES, applied to
$\boldsymbol{\delta}$ alone, answering whether a learned-covariance
continuous method outperforms our variable-specific mixed-integer GA
on the continuous subspace.
All baselines include Stage~2 global phase alignment
(Section~\ref{sec:stage2}) for a fair comparison.

\paragraph{B4: CMA-ES on $\boldsymbol{\delta}$.}
B4 runs CMA-ES~\cite{Hansen2001CMA} on $\boldsymbol{\delta} \in
[0.2, 0.8]^{2m}$ with $\boldsymbol{\kappa}$ fixed at $\mathbf{0}$ and
all other genotype variables fixed as in B0. Initial step size
$\sigma_0 = 0.15$ (one quarter of the domain span, as recommended by
pycma); population size is pycma's default
$\lambda = 4 + \lfloor 3 \ln n \rfloor$. Total evaluations are capped
at 2{,}000, matching the GA's budget. CMA-ES uses the same geometry
pipeline and Stage 2 solver as the GA, and the same scalar fitness
$F_\Sigma$ is minimised. B4 is the strongest off-the-shelf continuous
baseline for $\boldsymbol{\delta}$: any improvement of the GA over B4
is attributable either to the $\boldsymbol{\kappa}$ operator (which
CMA-ES cannot express) or to the population-based design tolerating
the piecewise-constant $\boldsymbol{\delta}$ landscape better than
CMA-ES's covariance adaptation.
...
\paragraph{GA$_{\text{no-}\kappa}$: Operator ablation.}
We also evaluate a GA variant in which the per-gene $\boldsymbol{\kappa}$
mutation probability is set to zero ($p_\kappa = 0$), so that phase
offsets evolve only through crossover and the initial population.
All other operators, the $\boldsymbol{\delta}$ Gaussian mutation, the
uniform crossover over all variable types, tournament selection with
Pareto dominance, and elitism, are left unchanged.
This isolates the contribution of the $\boldsymbol{\kappa}$ mutation
operator on top of $\boldsymbol{\delta}$ optimisation, answering whether
the mixed-integer operator design earns its complexity.
...
Four comparisons isolate the individual contributions of the search
components.
(i) B2 on pants is exact over $8^4$ phase assignments, and B2 on the
onesie is the best of $5 \times 10^5$ uniform random samples from
$8^{11}$, the tightest $\boldsymbol{\kappa}$-only baseline we can
afford; both B2 instances fix $\boldsymbol{\delta} = 0.5$, so the GA's
$f_\text{align}$ improvement over B2 is attributable to
$\boldsymbol{\delta}$ optimisation, which no $\boldsymbol{\kappa}$-only
method performs.
(ii) B3 searches the same joint $(\boldsymbol{\delta}, \boldsymbol{\kappa})$
space as the GA at the same evaluation budget, with the same geometry
pipeline and the same Stage~2 solver. The two methods pay the same
infeasibility tax (Section~\ref{sec:ea}): $55$--$60\,\%$ of attempted
evaluations fail with a death-penalty fitness in both cases, so neither
method enjoys a larger effective valid-evaluation budget. The GA's
improvement over B3 is therefore not attributable to implicit constraint
handling but to \emph{where} in the feasibility region its valid samples
are drawn from: selection pressure and mutation around elite parents
concentrate the GA's sampling near the current best, whereas B3 draws
uniformly from the full feasibility region and cannot exploit
previously-discovered good neighbourhoods. The resulting
$[X]\%$$^\dagger$ gap on $f_\text{align}$ quantifies this exploitation
advantage at matched budget.
(iii) B4 is an off-the-shelf CMA-ES applied to $\boldsymbol{\delta}$
alone with $\boldsymbol{\kappa} = 0$, again at matched evaluation
budget; any improvement of the GA over B4 is attributable either to
the $\boldsymbol{\kappa}$ operator, which a continuous optimiser
cannot express, or to population-based search tolerating the
piecewise-constant $\boldsymbol{\delta}$ landscape better than
covariance adaptation.
(iv) Removing $\boldsymbol{\kappa}$ mutation from the GA
($\text{GA}_{\text{no-}\kappa}$) isolates the contribution of the
$\boldsymbol{\kappa}$ operator on top of $\boldsymbol{\delta}$ search:
this variant inherits the full $\boldsymbol{\delta}$ landscape of the
GA but loses its ability to move $\boldsymbol{\kappa}$ beyond the initial
population, and is dominated by the full GA by $[X]\%$$^\dagger$ on
$f_\text{align}$, separating the continuous-subspace contribution from
the mixed-integer contribution within the GA itself.
Taken together, the four comparisons align with the landscape analysis
in Section~\ref{sec:ea}: the largest component of the GA's advantage
comes from $\boldsymbol{\delta}$ optimisation, which $\boldsymbol{\kappa}$
mutation refines, and which the evolutionary operators exploit more
reliably than either uniform sampling (B3) or a learned-covariance
continuous method (B4) on this discrete-plateau landscape.
% TODO: fill quantitative improvement percentages, GA_no-kappa gap, and precise f_fit bound once 10 seeds complete
...
\caption{
  ...
  B0/B1/B2 are deterministic baselines (single value reported).
  B3 (random search at GA budget), $\text{GA}_{\text{no-}\kappa}$
  (operator ablation) and GA report mean $\pm$ std over 10 seeds.
...
\caption{
  ...
  GA: mean $\pm$ std over 10 seeds.
  $^\dagger$Preliminary: fewer than 10 seeds completed.
}
