...
The fit guard $f_\text{fit}$ is near zero across runs, confirming
that the penalty (weighted at $w_\text{fit} = 10$) effectively prevents
garment shrinkage without requiring manual tuning of seam bounds.
...
$\boldsymbol{\kappa}$ explores regions of the search space that no
baseline can access.
