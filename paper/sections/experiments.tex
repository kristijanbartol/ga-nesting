...
B2 on pants is exact over $8^4$ phase assignments, and B2 on the onesie
is the best of $5 \times 10^5$ uniform random samples from $8^{11}$,
the tightest $\boldsymbol{\kappa}$-only baseline we can afford at the
GA's evaluation budget.
Both B2 instances fix $\boldsymbol{\delta} = 0.5$, so the GA's improvement
over B2 is attributable to $\boldsymbol{\delta}$ optimisation, which no
baseline performs; this is the effect the evolutionary search is designed
to exploit, and is what justifies its cost relative to
$\boldsymbol{\kappa}$-only alternatives.
% TODO: fill quantitative improvement percentages and precise f_fit bound once 20 seeds complete
